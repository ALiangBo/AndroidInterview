...
- 泛型中extends和super的区别
  ...
  extends 限定类型必须是给定类型的子类。super 限定类型必须是给定类型的父类。extends 和 super 的使用应该遵循 PECS 原则。即：如果是生产者则使用 extends ，如果是消费者则使用 super（简而言之就是如果只读则使用 extends ，只写则使用 super）。
...
- Java的异常体系
  ...
  java 的异常对象都是派生于 Throwable。应用程序的异常主要分为 RuntimeException 和 IOException.由于程序逻辑导致的异常为 RuntimeException;有 I/O 操作导致的异常为 IOException

- 谈谈你对解析与分派的认识。

  解析：调用目标方法在编译器进行编译时就必须确定下来，这类方法的调用称为解析。

  分派：静态分派和动态分派；静态分派是指一个类有多重载方法时，虚拟机在选择调用方法的时候会更具参数的静态类型来选择合适的方法，而不是通过参数的实际类型。静态分派发生在编译阶段，因此确定静态分配的动作实际上不是由虚拟机来执行的；动态分派是指虚拟机在运行时根据实际类型判断具体是调用父类的方法还是子类的方法重写。
...
- Java中实现多态的机制是什么？
  ...
  重载和重写。（重载和重写涉及到分派，可参考问题“谈谈你对解析与分派的认识”）
...
- String为什么要设计成不可变的？
  ...
  String 设计成不可变方便于代码维护。如果程序中多处使用了同一个 String 对象，而且都可以修改，那么程序对 String 的维护将变得困难。除非使用的时候新建一个 String ,但这样有可能会导致生成大量的 String.
